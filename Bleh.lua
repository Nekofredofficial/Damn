-- JANE GUI (ESP & CHAMS) 🔥 - INSPIRED BY JANE DOE
...
mainFrame.BackgroundColor3 = Color3.fromRGB(255, 200, 220) -- Pink background
...
-- Modern gradient background (Pink themed)
...
-- Glow effect border (Pink animated)
...
titleBar.BackgroundColor3 = Color3.fromRGB(255, 150, 180)
...
creatorText.Size = UDim2.new(0, 120, 0, 20)
creatorText.Position = UDim2.new(0, 15, 1, -22)
creatorText.Text = "inspired by Jane Doe"
creatorText.TextColor3 = Color3.fromRGB(255, 200, 220)
...
minimizeBtn.BackgroundColor3 = Color3.fromRGB(255, 120, 160)
...
minimizeBtn.MouseEnter:Connect(function()
    TweenService:Create(minimizeBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 100, 140)}):Play()
end)

minimizeBtn.MouseLeave:Connect(function()
    TweenService:Create(minimizeBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 120, 160)}):Play()
end)
...
-- Modern Page Navigation with animations
...
prevPageBtn.BackgroundColor3 = Color3.fromRGB(255, 120, 160)
...
-- Previous button hover animation
prevPageBtn.MouseEnter:Connect(function()
    TweenService:Create(prevPageBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 100, 140)}):Play()
end)

prevPageBtn.MouseLeave:Connect(function()
    TweenService:Create(prevPageBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 120, 160)}):Play()
end)
...
nextPageBtn.BackgroundColor3 = Color3.fromRGB(255, 120, 160)
...
nextPageBtn.MouseEnter:Connect(function()
    TweenService:Create(nextPageBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 100, 140)}):Play()
end)

nextPageBtn.MouseLeave:Connect(function()
    TweenService:Create(nextPageBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 120, 160)}):Play()
end)
...
pageIndicator.TextColor3 = Color3.fromRGB(255, 200, 220)
pageIndicator.Text = "1/7"
pageIndicator.TextSize = 16
pageIndicator.Font = Enum.Font.GothamBold

-- Create page frames with animation support
for page = 1, totalPages do
    ...
    pageFrame.ScrollBarImageColor3 = Color3.fromRGB(255, 150, 200)
...
minimizeBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(minimizeBtn, TweenInfo.new(0.2), {
        BackgroundColor3 = isMinimized and Color3.fromRGB(255, 100, 140) or Color3.fromRGB(255, 120, 160),
...
local function createModernButton(parent, text, isToggle, color)
    ...
    btn.BackgroundColor3 = color or Color3.fromRGB(255, 120, 160)
...
local shotThroughWallsConnection = nil
...
-- MOVEMENT FIX: Store original character state
...
-- =============================================
-- FIX MOVEMENT GLITCHES FUNCTION
-- =============================================
local function fixMovementGlitches()
    ...
    if humanoid then
        -- Reset platform stand to prevent floating
        ...
        if humanoid:GetState() ~= Enum.HumanoidStateType.Freefall and humanoid:GetState() ~= Enum.HumanoidStateType.Jumping then
            if rootPart and rootPart.Velocity.Y < -10 then
                rootPart.Velocity = Vector3.new(rootPart.Velocity.X, -5, rootPart.Velocity.Z)
            end
        end
        
        -- Prevent sliding on ground
        if humanoid.FloorMaterial ~= Enum.Material.Air and rootPart then
            local velocity = rootPart.Velocity
            if math.abs(velocity.X) < 0.5 and math.abs(velocity.Z) < 0.5 then
...
-- Character added handling with movement fix
LocalPlayer.CharacterAdded:Connect(function(character)
    ...
    if humanoid then
        ...
        if rootPart and rootPart.Velocity.Y > 0 and rootPart.Position.Y > 10 then
            rootPart.Velocity = Vector3.new(rootPart.Velocity.X, 0, rootPart.Velocity.Z)
        end
    end
    
    -- Fix movement glitches constantly
    local movementFixConnection = RunService.Heartbeat:Connect(function()
        fixMovementGlitches()
    end)
    
    character.AncestryChanged:Connect(function()
        if not character.Parent then
            movementFixConnection:Disconnect()
        end
    end)
end)

-- Regular movement fix loop
RunService.Heartbeat:Connect(function()
    fixMovementGlitches()
end)

-- =============================================
-- SHOT THROUGH WALLS FUNCTION (NEW)
-- =============================================
local function setupShotThroughWalls()
    -- This function allows projectiles (bullets, knives, etc.) to pass through walls
    -- Works by temporarily disabling collision between walls and projectiles
    
    local function enableWallsPass()
        -- Find all walls and obstacles in workspace
        for _, obj in pairs(workspace:GetDescendants()) do
            if obj:IsA("BasePart") and obj.CanCollide then
                ...
                -- Disable collision for walls to allow shooting through
                if obj.Name:match("Wall") or obj.Name:match("Floor") or obj.Name:match("Roof") or 
                   obj.Name:match("Barrier") or obj.Name:match("Block") or obj.Material == Enum.Material.Glass or
                   obj.Material == Enum.Material.Wood or obj.Material == Enum.Material.Concrete then
                    obj.CanCollide = false
                end
            end
        end
        
        -- Also handle tools/weapons to make them ignore walls
        if LocalPlayer.Character then
            local backpack = LocalPlayer.Backpack
            for _, tool in pairs(backpack:GetChildren()) do
                if tool:IsA("Tool") then
                    -- Disable handle collisions for weapons
                    local handle = tool:FindFirstChild("Handle")
                    if handle and handle:IsA("BasePart") then
                        originalCollisions[handle] = handle.CanCollide
    ...
    local function disableWallsPass()
    ...
    if shotThroughWallsEnabled then
        enableWallsPass()
        -- Keep checking for new walls
        shotThroughWallsConnection = RunService.RenderStepped:Connect(function()
            if shotThroughWallsEnabled then
                enableWallsPass()
            end
        end)
    else
        if shotThroughWallsConnection then
            shotThroughWallsConnection:Disconnect()
            shotThroughWallsConnection = nil
        end
        disableWallsPass()
    end
end

local function toggleShotThroughWalls()
    shotThroughWallsEnabled = not shotThroughWallsEnabled
    shotThroughWallsBtn.Text = "🔫 SHOT THROUGH WALLS" .. (shotThroughWallsEnabled and " [ON]" or " [OFF]")
    
    if shotThroughWallsEnabled then
        TweenService:Create(shotThroughWallsBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 50, 100)}):Play()
        setupShotThroughWalls()
        
        -- Create notification effect
        local notif = Instance.new("TextLabel", gui)
        notif.Size = UDim2.new(0, 300, 0, 50)
        notif.Position = UDim2.new(0.5, -150, 0.2, 0)
        notif.BackgroundColor3 = Color3.fromRGB(255, 100, 150)
        notif.TextColor3 = Color3.fromRGB(255, 255, 255)
        notif.Text = "🔫 SHOT THROUGH WALLS ACTIVATED!"
        notif.TextSize = 18
        ...
        TweenService:Create(shotThroughWallsBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(200, 100, 150)}):Play()
        setupShotThroughWalls()
        ...
        notif.BackgroundColor3 = Color3.fromRGB(255, 100, 150)
        notif.TextColor3 = Color3.fromRGB(255, 255, 255)
        notif.Text = "🔫 SHOT THROUGH WALLS DEACTIVATED!"
        notif.TextSize = 18
        notif.Font = Enum.Font.GothamBold
        notif.BackgroundTransparency = 0.3
        local notifCorner = Instance.new("UICorner", notif)
...
spawn(function()
    while true do
        TweenService:Create(combatTitle, TweenInfo.new(2), {TextTransparency = 0.2}):Play()
        task.wait(2)
        TweenService:Create(combatTitle, TweenInfo.new(2), {TextTransparency = 0}):Play()
        task.wait(2)
    end
end)
...
local combatButtons = {
    ...
    {"Rainbow ESP", true, Color3.fromRGB(255, 130, 170)},
    {"Chams", true, Color3.fromRGB(255, 110, 160)},
    {"Rainbow Chams", true, Color3.fromRGB(255, 140, 180)},
    {"Aimbot", true, Color3.fromRGB(255, 90, 140)},
    {"Aimbot Enemy Only", true, Color3.fromRGB(255, 150, 190)},
    {"Team Check", true, Color3.fromRGB(255, 120, 170)},
    {"Spinbot", true, Color3.fromRGB(255, 160, 200)}
}
...
movementTitle.TextColor3 = Color3.fromRGB(255, 150, 200)
...
spawn(function()
    while true do
        TweenService:Create(movementTitle, TweenInfo.new(2), {TextTransparency = 0.2}):Play()
        task.wait(2)
        TweenService:Create(movementTitle, TweenInfo.new(2), {TextTransparency = 0}):Play()
        task.wait(2)
    end
end)
...
local movementButtons = {
    {"Speed", true, Color3.fromRGB(255, 100, 150)},
    {"JumpPower", true, Color3.fromRGB(255, 120, 170)},
    {"Infinite Jump", true, Color3.fromRGB(255, 140, 190)},
    {"Infinite Stamina", true, Color3.fromRGB(255, 110, 160)},
    {"Noclip", true, Color3.fromRGB(255, 90, 140)},
    {"Auto Dodge", true, Color3.fromRGB(255, 150, 200)},
    {"Click TP", false, Color3.fromRGB(255, 130, 180)}
}
...
visualTitle.TextColor3 = Color3.fromRGB(255, 150, 200)
...
spawn(function()
    while true do
        TweenService:Create(visualTitle, TweenInfo.new(2), {TextTransparency = 0.2}):Play()
        task.wait(2)
        TweenService:Create(visualTitle, TweenInfo.new(2), {TextTransparency = 0}):Play()
        task.wait(2)
    end
end)
...
yPos = 40
local visualButtons = {
    {"XRay", true, Color3.fromRGB(255, 100, 150)},
    {"No Fog", true, Color3.fromRGB(255, 120, 170)},
    {"Full Bright", true, Color3.fromRGB(255, 140, 190)},
    {"Player Detector", true, Color3.fromRGB(255, 110, 160)}
}
...
teleportTitle.TextColor3 = Color3.fromRGB(255, 150, 200)
...
playerSearchBox.BackgroundColor3 = Color3.fromRGB(255, 150, 200)
playerSearchBox.TextColor3 = Color3.fromRGB(255, 255, 255)
playerSearchBox.PlaceholderText = "Enter player name..."
playerSearchBox.PlaceholderColor3 = Color3.fromRGB(200, 100, 150)
...
playerSearchBox.Focused:Connect(function()
    TweenService:Create(playerSearchBox, TweenInfo.new(0.2), {
        BackgroundColor3 = Color3.fromRGB(255, 130, 180),
...
local teleportBtn, teleportContainer = createModernButton(pageFrames[3], "Teleport to Player", false, Color3.fromRGB(255, 100, 150))
...
protectionTitle.TextColor3 = Color3.fromRGB(255, 150, 200)
...
spawn(function()
    while true do
        TweenService:Create(protectionTitle, TweenInfo.new(2), {TextTransparency = 0.2}):Play()
        task.wait(2)
        TweenService:Create(protectionTitle, TweenInfo.new(2), {TextTransparency = 0}):Play()
        task.wait(2)
    end
end)
...
godModeBtn.BackgroundColor3 = Color3.fromRGB(255, 80, 120)
...
antiDamageBtn.BackgroundColor3 = Color3.fromRGB(255, 100, 140)
...
protectionLabel.TextColor3 = Color3.fromRGB(255, 200, 220)
...
local protectionFeatures = {
    {"Anti Fall Damage", true, Color3.fromRGB(255, 120, 170)},
    {"Anti Explosion", true, Color3.fromRGB(255, 130, 180)},
    {"Anti Fire", true, Color3.fromRGB(255, 110, 160)},
    {"Anti Drown", true, Color3.fromRGB(255, 140, 190)}
}
...
aimbotSettingsTitle.TextColor3 = Color3.fromRGB(255, 150, 200)
...
aimbotInfo.TextColor3 = Color3.fromRGB(255, 200, 220)
...
currentTargetDisplay.BackgroundColor3 = Color3.fromRGB(255, 150, 200)
currentTargetDisplay.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local targetY = 135
local targetColors = {
    [1] = Color3.fromRGB(255, 80, 120),   -- Head - Pink/Red
    [2] = Color3.fromRGB(255, 120, 160),  -- Torso - Pink
    [3] = Color3.fromRGB(255, 100, 140),  -- Whole Character - Pink/Purple
    [4] = Color3.fromRGB(255, 140, 180)   -- Random - Light Pink
}
...
aimbotInfo2.TextColor3 = Color3.fromRGB(255, 200, 220)
...
-- =============================================
-- PAGE 6: WEAPON FEATURES (NEW - SHOT THROUGH WALLS)
-- =============================================
local weaponTitle = Instance.new("TextLabel", pageFrames[6])
weaponTitle.Size = UDim2.new(1, 0, 0, 35)
weaponTitle.Position = UDim2.new(0, 10, 0, 5)
weaponTitle.BackgroundTransparency = 1
weaponTitle.Text = "🔫 WEAPON FEATURES"
weaponTitle.TextColor3 = Color3.fromRGB(255, 100, 150)
weaponTitle.TextSize = 20
...
spawn(function()
    while true do
        TweenService:Create(weaponTitle, TweenInfo.new(2), {TextTransparency = 0.2}):Play()
        task.wait(2)
        TweenService:Create(weaponTitle, TweenInfo.new(2), {TextTransparency = 0}):Play()
        task.wait(2)
    end
end)

local weaponInfo = Instance.new("TextLabel", pageFrames[6])
weaponInfo.Size = UDim2.new(1, -20, 0, 50)
weaponInfo.Position = UDim2.new(0, 10, 0, 45)
weaponInfo.BackgroundTransparency = 1
weaponInfo.Text = "Shoot through walls! Bullets, knives, and projectiles will pass through obstacles."
weaponInfo.TextColor3 = Color3.fromRGB(255, 200, 220)
...
-- Shot Through Walls Button
local shotThroughWallsContainer = Instance.new("Frame", pageFrames[6])
shotThroughWallsContainer.Size = UDim2.new(1, -20, 0, 55)
shotThroughWallsContainer.Position = UDim2.new(0, 10, 0, 105)
shotThroughWallsContainer.BackgroundTransparency = 1

local shotThroughWallsBtn = Instance.new("TextButton", shotThroughWallsContainer)
shotThroughWallsBtn.Size = UDim2.new(1, 0, 1, 0)
shotThroughWallsBtn.BackgroundColor3 = Color3.fromRGB(200, 100, 150)
shotThroughWallsBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
shotThroughWallsBtn.Text = "🔫 SHOT THROUGH WALLS [OFF]"
shotThroughWallsBtn.TextSize = 16
shotThroughWallsBtn.Font = Enum.Font.GothamBold
shotThroughWallsBtn.BorderSizePixel = 0

local shotCorner = Instance.new("UICorner", shotThroughWallsBtn)
shotCorner.CornerRadius = UDim.new(0, 10)

shotThroughWallsBtn.MouseEnter:Connect(function()
    TweenService:Create(shotThroughWallsBtn, TweenInfo.new(0.2), {
        Size = UDim2.new(1.02, 0, 1.02, 0),
        Position = UDim2.new(-0.01, 0, -0.01, 0)
    }):Play()
end)

shotThroughWallsBtn.MouseLeave:Connect(function()
    TweenService:Create(shotThroughWallsBtn, TweenInfo.new(0.2), {
        Size = UDim2.new(1, 0, 1, 0),
        Position = UDim2.new(0, 0, 0, 0)
    }):Play()
...
shotThroughWallsBtn.MouseButton1Click:Connect(toggleShotThroughWalls)

local weaponWarning = Instance.new("TextLabel", pageFrames[6])
weaponWarning.Size = UDim2.new(1, -20, 0, 40)
weaponWarning.Position = UDim2.new(0, 10, 0, 175)
weaponWarning.BackgroundTransparency = 1
weaponWarning.Text = "💡 Tip: Use with any weapon! Works best with guns and throwables."
weaponWarning.TextColor3 = Color3.fromRGB(255, 180, 200)
weaponWarning.TextSize = 12
weaponWarning.Font = Enum.Font.Gotham
weaponWarning.TextWrapped = true
weaponWarning.TextXAlignment = Enum.TextXAlignment.Left

pageFrames[6].CanvasSize = UDim2.new(0, 0, 0, 230)
...
themeTitle.TextColor3 = Color3.fromRGB(255, 150, 200)
themeTitle.TextSize = 20
themeTitle.Font = Enum.Font.GothamBold
themeTitle.TextXAlignment = Enum.TextXAlignment.Left

-- Color themes (Pink themed)
local colorThemes = {
    ["Pink"] = {
        main = Color3.fromRGB(255, 200, 220),
        title = Color3.fromRGB(255, 150, 180),
        button = Color3.fromRGB(255, 120, 160),
        text = Color3.fromRGB(255, 255, 255)
    },
    ["Hot Pink"] = {
        main = Color3.fromRGB(255, 150, 200),
        title = Color3.fromRGB(255, 100, 150),
        button = Color3.fromRGB(255, 80, 130),
        text = Color3.fromRGB(255, 255, 255)
    },
    ["Light Pink"] = {
        main = Color3.fromRGB(255, 220, 235),
        title = Color3.fromRGB(255, 180, 210),
        button = Color3.fromRGB(255, 140, 180),
        text = Color3.fromRGB(255, 255, 255)
    },
    ["Rose"] = {
        main = Color3.fromRGB(255, 180, 200),
        title = Color3.fromRGB(255, 130, 160),
        button = Color3.fromRGB(255, 100, 140),
        text = Color3.fromRGB(255, 255, 255)
    },
    ["Cherry"] = {
        main = Color3.fromRGB(255, 160, 180),
        title = Color3.fromRGB(255, 110, 140),
        button = Color3.fromRGB(255, 90, 120),
        text = Color3.fromRGB(255, 255, 255)
    }
}
...
-- Function to apply color theme with animation
local function applyColorTheme(themeName)
    ...
    for _, btnData in ipairs(pageButtons) do
        if btnData.button and btnData.button:IsA("TextButton") then
            if not string.find(btnData.button.Text, "ON") and not string.find(btnData.button.Text, "GOD") and not string.find(btnData.button.Text, "ANTI") and not string.find(btnData.button.Text, "SHOT") then
...
local themeY = 40
local themeColorsList = {"Pink", "Hot Pink", "Light Pink", "Rose", "Cherry"}
...
spawn(function()
    while true do
        TweenService:Create(thankYouText, TweenInfo.new(2), {TextTransparency = 0.2}):Play()
        task.wait(2)
        TweenService:Create(thankYouText, TweenInfo.new(2), {TextTransparency = 0}):Play()
        task.wait(2)
    end
end)
...
creatorNameLabel.Text = "JANE DOE"
creatorNameLabel.TextColor3 = Color3.fromRGB(255, 100, 255)
creatorNameLabel.TextSize = 30
creatorNameLabel.Font = Enum.Font.GothamBold

-- Animate creator name rainbow
spawn(function()
    local hue = 0
    while true do
        hue = (hue + 0.01) % 1
...
versionText.Position = UDim2.new(0, 10, 0, 360)
versionText.BackgroundTransparency = 1
versionText.Text = "Version 2.0 | Inspired by Jane Doe"
versionText.TextColor3 = Color3.fromRGB(150, 150, 150)
versionText.TextSize = 14
versionText.Font = Enum.Font.Gotham

pageFrames[7].CanvasSize = UDim2.new(0, 0, 0, 420)

-- Apply default theme
applyColorTheme("Pink")

-- =============================================
-- MODIFIED GET TARGET FUNCTION WITH AIMBOT SETTINGS
-- =============================================
...
if success and Drawing then
    ...
    aimbotCircle.Color = Color3.fromRGB(255, 100, 150)
...
local function createDetectionLabel()
    ...
    label.BackgroundColor3 = Color3.fromRGB(255, 150, 200)
...
local function toggleGodMode()
    ...
    if godModeEnabled then
        TweenService:Create(godModeBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 50, 100)}):Play()
        ...
        TweenService:Create(godModeBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 80, 120)}):Play()
...
local function toggleAntiDamage()
    ...
    if antiDamageEnabled then
        TweenService:Create(antiDamageBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 70, 120)}):Play()
        ...
        TweenService:Create(antiDamageBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 100, 140)}):Play()
    end
end

-- =============================================
-- BUTTON CLICK CONNECTIONS (All existing)
-- =============================================
...
espBtn.MouseButton1Click:Connect(function()
    ...
    if espEnabled then
        TweenService:Create(espBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 50, 100)}):Play()
    else
        ...
        TweenService:Create(rainbowEspBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 130, 170)}):Play()
        chamsEnabled = false
        chamsBtn.Text = "Chams [OFF]"
        TweenService:Create(chamsBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 110, 160)}):Play()
        rainbowChamsEnabled = false
        rainbowChamsBtn.Text = "Rainbow Chams [OFF]"
        TweenService:Create(rainbowChamsBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 140, 180)}):Play()
    end
end)
...
chamsBtn.MouseButton1Click:Connect(function()
    ...
    if chamsEnabled then
        TweenService:Create(chamsBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 50, 100)}):Play()
    else
        TweenService:Create(chamsBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 110, 160)}):Play()
        rainbowChamsEnabled = false
        rainbowChamsBtn.Text = "Rainbow Chams [OFF]"
...
aimbotBtn.MouseButton1Click:Connect(function()
    ...
    if aimbotEnabled then
        TweenService:Create(aimbotBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 50, 100)}):Play()
    else
        TweenService:Create(aimbotBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 90, 140)}):Play()
        aimbotEnemyOnlyEnabled = false
        aimbotEnemyOnlyBtn.Text = "Aimbot Enemy Only [OFF]"
        TweenService:Create(aimbotEnemyOnlyBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 150, 190)}):Play()
    end
end)
...
aimbotEnemyOnlyBtn.MouseButton1Click:Connect(function()
    ...
    if aimbotEnemyOnlyEnabled then
        TweenService:Create(aimbotEnemyOnlyBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 100, 140)}):Play()
    else
        TweenService:Create(aimbotEnemyOnlyBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 150, 190)}):Play()
    end
end)
...
teamCheckBtn.MouseButton1Click:Connect(function()
    ...
    if teamCheckEnabled then
        TweenService:Create(teamCheckBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 70, 120)}):Play()
    else
        TweenService:Create(teamCheckBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 120, 170)}):Play()
    end
end)
...
spinbotBtn.MouseButton1Click:Connect(function()
    ...
    if spinbotEnabled then
        TweenService:Create(spinbotBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 100, 150)}):Play()
    else
        TweenService:Create(spinbotBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 160, 200)}):Play()
    end
end)
...
speedBtn.MouseButton1Click:Connect(function()
    ...
    if speedEnabled then
        TweenService:Create(speedBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 50, 100)}):Play()
    else
        TweenService:Create(speedBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 100, 150)}):Play()
    end
...
jumpPowerBtn.MouseButton1Click:Connect(function()
    ...
    if jumpPowerEnabled then
        TweenService:Create(jumpPowerBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 70, 120)}):Play()
    else
        TweenService:Create(jumpPowerBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 120, 170)}):Play()
    end
...
infiniteJumpBtn.MouseButton1Click:Connect(function()
    ...
    if infiniteJumpEnabled then
        TweenService:Create(infiniteJumpBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 90, 140)}):Play()
    else
        TweenService:Create(infiniteJumpBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 140, 190)}):Play()
    end
end)
...
infiniteStaminaBtn.MouseButton1Click:Connect(function()
    ...
    if infiniteStaminaEnabled then
        TweenService:Create(infiniteStaminaBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 60, 110)}):Play()
    else
        TweenService:Create(infiniteStaminaBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 110, 160)}):Play()
    end
end)
...
noclipBtn.MouseButton1Click:Connect(function()
    ...
    if noclipEnabled then
        TweenService:Create(noclipBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 50, 100)}):Play()
    else
        TweenService:Create(noclipBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 90, 140)}):Play()
    end
end)
...
autoDodgeBtn.MouseButton1Click:Connect(function()
    ...
    if autoDodgeEnabled then
        TweenService:Create(autoDodgeBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 80, 130)}):Play()
    else
        TweenService:Create(autoDodgeBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 150, 200)}):Play()
    end
end)
...
clickTpBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(clickTpBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 50, 150)}):Play()
    
    task.delay(2, function() 
        clickTpBtn.Text = "Click TP"
        TweenService:Create(clickTpBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 130, 180)}):Play()
    end)
end)
...
xrayBtn.MouseButton1Click:Connect(function()
    ...
    if xrayEnabled then
        TweenService:Create(xrayBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 60, 110)}):Play()
        applyXRay()
    else
        TweenService:Create(xrayBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 100, 150)}):Play()
        removeXRay()
    end
end)
...
noFogBtn.MouseButton1Click:Connect(function()
    ...
    if noFogEnabled then
        TweenService:Create(noFogBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 70, 120)}):Play()
        removeFog()
    else
        TweenService:Create(noFogBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 120, 170)}):Play()
        restoreFog()
    end
end)
...
fullBrightBtn.MouseButton1Click:Connect(function()
    ...
    if fullBrightEnabled then
        TweenService:Create(fullBrightBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 90, 140)}):Play()
        applyFullBright()
    else
        TweenService:Create(fullBrightBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 140, 190)}):Play()
        removeFullBright()
    end
end)
...
teleportToPlayerBtn.MouseButton1Click:Connect(function()
    ...
    if searchText == "" then
        ...
        task.delay(2, function() 
            ...
            TweenService:Create(teleportToPlayerBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 100, 150)}):Play()
        end)
        return
    end
...
antiFallDamageBtn.MouseButton1Click:Connect(function()
    ...
    if antiFallDamageEnabled then
        TweenService:Create(antiFallDamageBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 70, 120)}):Play()
    else
        TweenService:Create(antiFallDamageBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 120, 170)}):Play()
    end
end)
...
antiExplosionBtn.MouseButton1Click:Connect(function()
    ...
    if antiExplosionEnabled then
        TweenService:Create(antiExplosionBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 80, 130)}):Play()
    else
        TweenService:Create(antiExplosionBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 130, 180)}):Play()
    end
end)
...
antiFireBtn.MouseButton1Click:Connect(function()
    ...
    if antiFireEnabled then
        TweenService:Create(antiFireBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 60, 110)}):Play()
    else
        TweenService:Create(antiFireBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 110, 160)}):Play()
    end
end)
...
antiDrownBtn.MouseButton1Click:Connect(function()
    ...
    if antiDrownEnabled then
        TweenService:Create(antiDrownBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 90, 140)}):Play()
    else
        TweenService:Create(antiDrownBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(255, 140, 190)}):Play()
    end
end)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    -- Shot Through Walls - Keep applying
    if shotThroughWallsEnabled then
        setupShotThroughWalls()
    end
end)
...
-- Animated heart particles for Jane Doe theme
spawn(function()
    while true do
        if currentPage == 7 then
            local heart = Instance.new("TextLabel", pageFrames[7])
            ...
            TweenService:Create(heart, TweenInfo.new(2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Position = UDim2.new(heart.Position.X.Scale, heart.Position.X.Offset, heart.Position.Y.Scale - 0.2, heart.Position.Y.Offset),
                TextTransparency = 1
            }):Play()
...
gui.Destroying:Connect(function()
    ...
    if shotThroughWallsConnection then shotThroughWallsConnection:Disconnect() end
